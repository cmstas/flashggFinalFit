...
model_bkg(){
	pushd Trees2WS
	 python trees2ws_data.py --inputConfig syst_config_2HDM_M300.py --inputTreeFile $trees/Data/allData.root
	popd
	
	pushd Background
	 	rm -rf outdir_$tag
		sed -i "s/dummy/${tag}/g" config_2HDM_M300.py 

	  python RunBackgroundScripts.py --inputConfig config_2HDM_M300.py --mode fTestParallel

		sed -i "s/${tag}/dummy/g" config_2HDM_M300.py
	popd
}
...
model_sig(){
	...
	do
		...
		do
			...
			pushd Trees2WS
				python trees2ws.py --inputConfig syst_config_2HDM_M300.py --inputTreeFile $trees/$year/${proc}_125_13TeV.root --inputMass 125 --productionMode $proc --year $year --doSystematics
			popd
		...
		pushd Signal	
		    ...
		    sed -i "s/dummy/${tag}/g" syst_config_2HDM_M300_$year.py

                    python RunSignalScripts.py --inputConfig syst_config_2HDM_M300_$year.py --mode fTest --modeOpts "--doPlots"
		    python RunSignalScripts.py --inputConfig syst_config_2HDM_M300_$year.py --mode calcPhotonSyst
		    python RunSignalScripts.py --inputConfig syst_config_2HDM_M300_$year.py --mode signalFit --groupSignalFitJobsByCat --modeOpts "--skipVertexScenarioSplit "

	            sed -i "s/${tag}/dummy/g" syst_config_2HDM_M300_$year.py
	    	popd
	done
...
run_combine(){
	pushd Combine
		...
		python RunText2Workspace.py --mode  2HDM_M300 --dryRun
		./t2w_jobs/t2w_2HDM_M300.sh

		combine --expectSignal 1 -t -1 --redefineSignalPOI r --cminDefaultMinimizerStrategy 0 -M AsymptoticLimits -m 125 -d Datacard_2HDM_M300.root -n _AsymptoticLimit_r --freezeParameters MH --run=blind > combine_results_${tag}.txt
		combine --expectSignal 1 -t -1 --redefineSignalPOI r --cminDefaultMinimizerStrategy 0 -M AsymptoticLimits -m 125 -d Datacard_2HDM_M300.root -n _AsymptoticLimit_r --freezeParameters allConstrainedNuisances --run=blind > stat_only_${tag}.txt
