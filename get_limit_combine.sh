...
#  if [[ -n $(python should_do_impacts.py ${mx} ${my} ${mh}) ]]; then
    ...
    python ~/XYH/FinalFit/CMSSW_10_2_13/bin/slc7_amd64_gcc700/combineTool.py -t -1 --redefineSignalPOI r --cminDefaultMinimizerStrategy 0 -M Impacts -m ${mh} -d Datacard_ggtt_resonant_${m}_ggtt_resonant.root --freezeParameters MH,MX,MY,${index_names} --setParameters MX=${mx},MY=${my},r=${exp_limit}${index_values} --doInitialFit -n ${mo}
    python ~/XYH/FinalFit/CMSSW_10_2_13/bin/slc7_amd64_gcc700/combineTool.py -t -1 --redefineSignalPOI r --cminDefaultMinimizerStrategy 0 -M Impacts -m ${mh} -d Datacard_ggtt_resonant_${m}_ggtt_resonant.root --freezeParameters MH,MX,MY,${index_names} --setParameters MX=${mx},MY=${my},r=${exp_limit}${index_values} --doFits --parallel 8 -n ${mo}
    python ~/XYH/FinalFit/CMSSW_10_2_13/bin/slc7_amd64_gcc700/combineTool.py -M Impacts -d Datacard_ggtt_resonant_${m}_ggtt_resonant.root -m ${mh} -o impacts_${mo}.json -n ${mo}
