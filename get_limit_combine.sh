...
pushd Combine  
  ...
  #  echo "Runninb combine without ABCD correction"
    combine -v 2 --redefineSignalPOI r --cminDefaultMinimizerStrategy 0 -M AsymptoticLimits -m ${mh} -d Datacard_ggtt_resonant_${m}_ggtt_resonant.root -n _AsymptoticLimit_r_ggtt_resonant_${mo} --freezeParameters MH,MX,MY --freezeNuisanceGroups ABCD --run=expected --setParameters MX=${mx},MY=${my},dy_bkg_scaler=0 > combine_results_ggtt_resonant_no_dy_bkg_${mo}.txt
  #fi

  #echo "Running combine with ABCD"
  #combine -v 2 --redefineSignalPOI r --cminDefaultMinimizerStrategy 0 --X-rtd MINIMIZER_freezeDisassociatedParams --X-rtd MINIMIZER_multiMin_hideConstants --X-rtd MINIMIZER_multiMin_maskConstraints --X-rtd MINIMIZER_multiMin_maskChannels=2 -M AsymptoticLimits -m ${mh} -d Datacard_ggtt_resonant_${m}_ggtt_resonant.root -n _AsymptoticLimit_r_ggtt_resonant_${mo} --freezeParameters MH,MX,MY --run=expected --setParameters MX=${mx},MY=${my} > combine_results_ggtt_resonant_${mo}_fixed_C.txt
  #combine --redefineSignalPOI r --cminDefaultMinimizerStrategy 0 --X-rtd MINIMIZER_freezeDisassociatedParams --X-rtd MINIMIZER_multiMin_hideConstants --X-rtd MINIMIZER_multiMin_maskConstraints --X-rtd MINIMIZER_multiMin_maskChannels=2 -M AsymptoticLimits -m ${mh} -d Datacard_ggtt_resonant_${m}_ggtt_resonant.root -n _AsymptoticLimit_r_ggtt_resonant_${mo} --freezeParameters MH,MX,MY,r --run=expected --setParameters MX=${mx},MY=${my},r=0.0 > combine_results_ggtt_resonant_${mo}_background_only.txt
