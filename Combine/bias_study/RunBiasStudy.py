...
if opts.plots:
    ...
    for ipdf,pdfName in indexNameMap.iteritems():
        ...
        rHist = r.TH1F('r_%s'%name, 'r using the envelope to fit %s'%name, 25, -100., 100.)
        rHist.GetXaxis().SetTitle('r')
        rHist.GetYaxis().SetTitle('Entries')
        ...
        canv = r.TCanvas()
        rHist.Draw()
        canv.SaveAs('%s_%s.pdf'%(plotName(name),"r_value"))
        canv.SaveAs('%s_%s.png'%(plotName(name),"r_value"))
