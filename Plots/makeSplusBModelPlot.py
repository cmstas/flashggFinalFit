...
if opt.loadWeights != '':
  ...
  for c in cats:
    if c not in catsWeights: 
      print (" --> [ERROR] category %s is not in S/S+B weights json. Leaving..."%c)
      leave()
else:
...
if opt.doBands:
  if opt.loadToyYields != '':
    ...
    toyFiles = glob.glob("./SplusBModels%stoys/toy_*.root"%opt.ext)
    if len(toyFiles) == 0:
      print ("     * [ERROR] No toys files of form ./SplusBModels%stoys/toy_*.root. Skipping bands"%opt.ext)
      opt.doBands = False
    else:
      for tidx in range(len(toyFiles)):
        ...
        for cidx in range(chan.numTypes()):
          ...
          if( opt.cats == 'all' )|( c in opt.cats.split(",") ):
            ...
            dtoy = toy.reduce("CMS_channel==CMS_channel::{}".format(c)) 
      ...
      if opt.saveToyYields:
        print ("      * Saving toy yields to: SplusBModels%stoyYields_%s.pkl"%(opt.ext,opt.xvar.split(",")[0]))
        with open("SplusBModels%stoyYields_%s.pkl"%(opt.ext,opt.xvar.split(",")[0]),"wb") as fD: pickle.dump(df_bands,fD)
