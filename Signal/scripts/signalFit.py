...
if opt.skipZeroes:
  ...
  d = reduceDataset(inputWS.data("%s_%s_%s_%s"%(procToData(opt.proc.split("_201")[0]),MHNominal,sqrts__,opt.cat)),aset)
...
for mp in opt.massPoints.split(","):
  ...
  f = ROOT.TFile(WSFileName,"read")
  inputWS = f.Get(inputWSName__)
  d = reduceDataset(inputWS.data("%s_%s_%s_%s"%(procToData(procRVFit.split("_201")[0]),mp,sqrts__,catRVFit)),aset)
...
if( datasetRVForFit[MHNominal].numEntries() < opt.replacementThreshold  )|( datasetRVForFit[MHNominal].sumEntries() < 0. ):
  ...
  for mp in opt.massPoints.split(","):
    ...
    f = ROOT.TFile(WSFileName,"read")
    inputWS = f.Get(inputWSName__)
    d = reduceDataset(inputWS.data("%s_%s_%s_%s"%(procToData(procReplacementFit.split("_201")[0]),mp,sqrts__,catReplacementFit)),aset)
    if opt.skipVertexScenarioSplit: datasetRVForFit[mp] = d
    else: datasetRVForFit[mp] = splitRVWV(d,aset,mode="RV")
    inputWS.Delete()
    f.Close() 
...
if not opt.skipVertexScenarioSplit:
  ...
  for mp in opt.massPoints.split(","):
    ...
    f = ROOT.TFile(WSFileName,"read")
    inputWS = f.Get(inputWSName__)
    d = reduceDataset(inputWS.data("%s_%s_%s_%s"%(procToData(procWVFit.split("_201")[0]),mp,sqrts__,catWVFit)),aset)
